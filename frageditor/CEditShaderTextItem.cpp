...
//-----------------------------------------------------------------------
void CEditShaderTextItem::updateText()
{
    if (m_pCurrentShader)
    {
        QString code;
...
//-----------------------------------------------------------------------
void CEditShaderTextItem::setMessage(bool bIsError, const QString& message)
{
    m_pConsole->setTextColor(bIsError ? Qt::red : Qt::green);
...
void GLSLSyntaxHighlighter::highlightBlock(const QString &text)
{
    ...
    for (int i = 0; i < text.length(); ++i)
    {
        if (state == InsideCStyleComment)
        {
            ...
            {
                state = NormalState;
                setFormat(start, i - start + 2, m_CommentFormat);
            }
        }
        else
        {
