...
QList<AEntity*> CAssimpImporter::mergeScene(const QString& fileName, CSceneManager* pSceneManager, bool bMergeMaterials, CSceneNode* pNode /*= 0*/)
{
    ...
    if (!fileName.isEmpty())
    {
        ...
        {
            QByteArray biteArray = file.readAll();
            const aiScene* pAssimpScene = importer.ReadFileFromMemory(biteArray.constData(), biteArray.size(), aiProcessPreset_TargetRealtime_Quality);
