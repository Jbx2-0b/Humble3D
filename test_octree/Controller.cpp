    ...
    , m_LastPan(-1, -1)
{
    ...
    m_pLabel = new QLabel(m_pView);
    m_pLabel->resize(200, 60);
    m_pLabel->move(10, 10);
    m_pLabel->setStyleSheet("QLabel { color : white; }");

    pView->setGeometry(QRect(1920, 100, 800, 600));
}
...
{
    qDebug() << "DrawCount: " << m_pView->getGLRenderer()->getDrawCallCount();
    m_pLabel->setText(QString("FPS: %1\nDraw count: %2").arg(iFPS).arg(m_pView->getGLRenderer()->getDrawCallCount()));
}
