...
void QXmlPut::putDouble(const QString &tagName, double value)
{
	...
	mCurrentParent.appendChild(el);
	mCurrentElement = el;
	el.appendChild(mDocument.createTextNode(QString::number(value, 'g', 15)));
}
...
{
	mCurrentElement.setAttribute(name, QString::number(value, 'g', 15));
}
...
\endcode
*/
void QXmlPut::setAttributeDateTime(const QString &name, const QDateTime &value)
{
	mCurrentElement.setAttribute(name, value.toString(Qt::ISODate));
}
...
/*!
Returns the variant list the current tag carries as attribute with the name \a name. If the current tag
doesn't contain such an attribute,\a defaultValue is returned.
*/
QVariantList QXmlGet::getAttributeVariantList(const QString &name, const QVariantList &defaultValue) const
{
	if (mCurrentElement.hasAttribute(name))
	{
		QString value = mCurrentElement.attribute(name);
		return strToVariantList(value);
	}
	return defaultValue;
}
...
/*!
Returns the QDateTime the current tag carries as attribute with the name \a name. If the current tag
doesn't contain such an attribute,\a defaultValue is returned.
*/
QDateTime QXmlGet::getAttributeDateTime(const QString &name, const QDateTime &defaultValue) const
{
	if (mCurrentElement.hasAttribute(name))
	{
		QString value = mCurrentElement.attribute(name);
...
QVariantList QXmlGet::strToVariantList(QString value)
{
	QStringList valueList = value.split(";");
	QVariantList result;
	for (int i=0; i<valueList.size(); ++i)
	{
		result.append(valueList.at(i));
	}
	return result;
}
